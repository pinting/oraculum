...
def main() -> None:
    try:
        try:
            ...
            vocabulary: kl.Vocabulary = kl.Vocabulary.from_file_path("../vocabulary.tiktoken", 1)
        ...
        ac_base: kl.AhoCorasick = kl.AhoCorasick(vocabulary)
        t1 = time.perf_counter()

        print(f"Lattice base (AhoCorasick) built ({(t1 - t0) * 1000:.2f} ms)")

        t0 = time.perf_counter()
        toktrie_base: kl.TokTrie = kl.TokTrie(vocabulary)
        t1 = time.perf_counter()

        print(f"Expression base (TokTrie) built ({(t1 - t0) * 1000:.2f} ms)")

        indexes: list[kl.Lattice | kl.Expression] = []

        input_str: str = "Why "

        t0 = time.perf_counter()
        index = kl.Lattice(input_str, vocabulary, ac_base)
        t1 = time.perf_counter()
        ...
        t0 = time.perf_counter()
        index = kl.Expression(input_str, vocabulary, toktrie_base)
        t1 = time.perf_counter()
