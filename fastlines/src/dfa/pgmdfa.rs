...
pub struct PGMDFA<N, T> {
    index: Option<PGMIndex<u64>>,
    offsets: Offsets,
    tokens: Vec<T>,
    targets: Vec<N>,
    stride: u64,
...
impl<N, T> DFA<N, T> for PGMDFA<N, T>
where N: Number, T: Number {
    fn new(m: HashMap<N, HashMap<T, N>>, nodes_count: usize) -> Self {
        ...
        let stride = tokens.iter()
            .map(|t| t.to_usize() as u64)
            .max()
            .unwrap_or(0) + 1;

        let index = if !transitions.is_empty() {
            let composite_keys: Vec<u64> = transitions.iter()
                .map(|(src, token, _)| src.to_usize() as u64 * stride + token.to_usize() as u64)
                .collect();

            Some(PGMIndex::new(composite_keys, 128))
        } else {
            None
        };
        ...
        Self { index, offsets, tokens, targets, stride }
    }

    #[inline(always)]
    fn next(&self, src: N, transition: T) -> Option<N> {
        if let Some(index) = self.index.as_ref() {
            let key = src.to_usize() as u64 * self.stride + transition.to_usize() as u64;

            if let Some(pos) = index.get(key) {
                return Some(self.targets[pos]);
            }
        }
