...
where N: Number, T: Number, O: Number {
    ...
    fn lookup(&self, src: N, token: T) -> Option<N> {
        ...
        match header.unit_size {
             8 => search::<u8, N, T>(index_ptr, index_offset, data_offset, section_mask, token, &self.tokens, &self.targets),
            16 => search::<u16, N, T>(index_ptr, index_offset, data_offset, section_mask, token, &self.tokens, &self.targets),
            32 => search::<u32, N, T>(index_ptr, index_offset, data_offset, section_mask, token, &self.tokens, &self.targets),
            64 => search::<u64, N, T>(index_ptr, index_offset, data_offset, section_mask, token, &self.tokens, &self.targets),
             _ => unreachable!("Invalid unit size"),
        }
    }
